...
resource "aws_iam_group_membership" "managers_members" {
  ...
  users = [
    for user in aws_iam_user.users : user.name if contains(keys(user.tags), "JobTitle") && can(regex("Manager|CEO|VP|Director", user.tags.JobTitle))
  ]
}

resource "aws_iam_group_membership" "finance_members" {
